...
def restrict_admin_access():
    ...
    if request.path.startswith('/api/admin/'):
        # Check remote_addr, ignore X-Forwarded-For to prevent spoofing
        if request.remote_addr not in ('127.0.0.1', '::1'):
            return jsonify({'status': 'error', 'message': 'Forbidden: Admin endpoints are restricted to local access.'}), 403
